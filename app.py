    ...
    with pdfplumber.open(pdf_file) as pdf:
        ...
        if date:  # Only add row if valid date exists
            data.append({
                ...
                "Chemicals (bbl)": float(chem_match.group(1)) if chem_match else 0,
                "SCE (bbl)": float(sce_match.group(1)) if sce_match else 0,
                "Misc Other Loss (bbl)": float(misc_match.group(1)) if misc_match else 0,
                "Date": date,
...
if uploaded_files:
    ...
    if not combined_df.empty:
        ...
        st.dataframe(combined_df[[
            "Date", "Mud Weight", "PV", "YP", "Electrical Stability",
            "Base (bbl)", "Water (bbl)", "Barite (bbl)", "Chemicals (bbl)",
            "SCE (bbl)", "Misc Other Loss (bbl)",
            "Additions (bbl)", "Losses (bbl)"
        ]])
